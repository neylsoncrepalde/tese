...
\begin{document}
	...
	To \citeonline{eloire2009reseaux}, the main goal of White's model is to show the structure of the market. The market plane shows six zones which can be described in a simplified way\footnote{A mathematical development of this plane is described in Appendix 1.} like this: zone C is labelled ``ordinary''. In this zone, revenues decrease to scale ($a/c < 1$) since cost increases with quality ($d > 1$). Zone D is labelled ``advanced''. In this zone revenues increase to scale ($a/c > 1$ and $d > 0$). Zone A is labelled ``paradox''. In this zone cost does not increase with quality ($d < 0$ and $a/c < 1$). In this sense ...  % parei no final da página 61 da tese de Eloire
